#pragma once
#include "Config.h"
#include "Theory.h"

#if defined(ESP_ARDUINO_VERSION_MAJOR) && ESP_ARDUINO_VERSION_MAJOR >= 3
  #include <ESP_I2S.h>
  static I2SClass i2s;

  static bool audioBegin() {
    i2s.setPins(PIN_I2S_BCLK, PIN_I2S_LRC, PIN_I2S_DIN);
    return i2s.begin(I2S_MODE_STD, SAMPLE_RATE,
                     I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO);
  }
  static void audioWrite(int16_t* buf, size_t frames) {
    i2s.write((uint8_t*)buf, frames * 2 * sizeof(int16_t));
  }
#else
  #include <driver/i2s.h>

  static bool audioBegin() {
    i2s_config_t cfg = {
      .mode                 = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX),
      .sample_rate          = SAMPLE_RATE,
      .bits_per_sample      = I2S_BITS_PER_SAMPLE_16BIT,
      .channel_format       = I2S_CHANNEL_FMT_RIGHT_LEFT,
      .communication_format = I2S_COMM_FORMAT_STAND_I2S,
      .intr_alloc_flags     = 0,
      .dma_buf_count        = 6,
      .dma_buf_len          = 256,
      .use_apll             = false,
      .tx_desc_auto_clear   = true,
      .fixed_mclk           = 0
    };
    if (i2s_driver_install(I2S_NUM_0, &cfg, 0, NULL) != ESP_OK) return false;
    i2s_pin_config_t pins = {
      .mck_io_num   = I2S_PIN_NO_CHANGE,
      .bck_io_num   = PIN_I2S_BCLK,
      .ws_io_num    = PIN_I2S_LRC,
      .data_out_num = PIN_I2S_DIN,
      .data_in_num  = I2S_PIN_NO_CHANGE
    };
    return i2s_set_pin(I2S_NUM_0, &pins) == ESP_OK;
  }
  static void audioWrite(int16_t* buf, size_t frames) {
    size_t written;
    i2s_write(I2S_NUM_0, buf, frames * 2 * sizeof(int16_t), &written, portMAX_DELAY);
  }
#endif

enum Wave : uint8_t { W_SINE, W_TRI, W_SAW, W_SQR };

struct Instrument {
  const char* name;
  Wave  wave;
  float det;           
  float a, d, s, r;       
  float cut, env;       
  float wet;
  float sub;           
};

static const Instrument INSTRUMENTS[6] = {
  { "Warm Pad", W_SAW,  9,  0.35f,  0.5f,  0.75f, 1.2f,  1100, 900,  0.45f, 0.25f },
  { "E. Piano", W_TRI,  4,  0.005f, 1.1f,  0.15f, 0.5f,  2600, 2200, 0.28f, 0.35f },
  { "Organ",    W_SQR,  2,  0.01f,  0.05f, 1.0f,  0.12f, 1900, 0,    0.18f, 0.30f },
  { "Pluck",    W_SAW,  14, 0.002f, 0.35f, 0.0f,  0.35f, 3200, 2800, 0.35f, 0.20f },
  { "Glass",    W_SINE, 6,  0.01f,  0.9f,  0.2f,  0.9f,  4200, 1500, 0.55f, 0.15f },
  { "Bass",     W_SQR,  3,  0.005f, 0.4f,  0.6f,  0.25f, 800,  600,  0.10f, 0.45f }
};
static const uint8_t INSTRUMENT_COUNT = 6;

static const int AUDIO_FRAMES = 256;            

static const float CLIP_KNEE      = 0.70f;
static const float CLIP_SPAN      = 1.0f - CLIP_KNEE;
static const float CLIP_INV_SPAN  = 1.0f / CLIP_SPAN;


uint8_t           vol           = VOL_DEFAULT;
volatile float    gVolume       = VOL_LEVELS[VOL_DEFAULT];
volatile uint32_t gUnderruns    = 0;
volatile uint8_t  gVoicesActive = 0;   
static bool       audioOk       = false;

enum EvtType : uint8_t { EV_CHORD_ON, EV_CHORD_OFF };

struct AudioEvt {
  EvtType type;
  uint8_t src;        
  uint8_t inst;
  uint8_t count;    
  int16_t midi[5];
};
static QueueHandle_t evtQ = nullptr;

static const uint16_t SINE_BITS = 10;
static const uint16_t SINE_LEN  = 1 << SINE_BITS;  
static float sineTab[SINE_LEN + 1];                 

static void sineTabInit() {
  for (uint16_t i = 0; i <= SINE_LEN; i++) sineTab[i] = sinf(TWO_PI * i / SINE_LEN);
}

static inline float oscSine(uint32_t ph) {
  uint32_t idx  = ph >> (32 - SINE_BITS);
  float    frac = (float)(ph & ((1UL << (32 - SINE_BITS)) - 1))
                  * (1.0f / (float)(1UL << (32 - SINE_BITS)));
  return sineTab[idx] + (sineTab[idx + 1] - sineTab[idx]) * frac;
}

static inline float polyBlep(float t, float dt) {
  if (t < dt)        { t /= dt;               return t + t - t * t - 1.0f; }
  if (t > 1.0f - dt) { t = (t - 1.0f) / dt;   return t * t + t + t + 1.0f; }
  return 0.0f;
}

static inline float oscShape(Wave w, uint32_t ph, float dt) {
  float t = (float)ph * (1.0f / 4294967296.0f);   
  if (w == W_SINE) return oscSine(ph);
  if (w == W_SAW)  return (2.0f * t - 1.0f) - polyBlep(t, dt);
  float v = (t < 0.5f) ? 1.0f : -1.0f;           
  v += polyBlep(t, dt);
  v -= polyBlep((t < 0.5f) ? t + 0.5f : t - 0.5f, dt);
  return v;
}


enum EnvStage : uint8_t { ENV_IDLE, ENV_ATTACK, ENV_DECAY, ENV_SUSTAIN, ENV_RELEASE };

struct Voice {
  bool     active;
  uint8_t  src;             
  Wave     wave;
  bool     isBass;

  uint32_t ph[2], inc[2];  
  float    dt[2];           
  uint8_t  oscCount;
  uint32_t phSub, incSub;
  float    subGain;
  float    triZ[2];         

  EnvStage stage;
  float    env, peak, sustainLevel;
  float    aStep, dStep, rStep;  

  float    fCut, fTarget, fCoefBlk; 
  float    fG;                      
  float    lp1, lp2;               
};

static Voice voices[MAX_VOICES];

static inline uint32_t incForHz(float hz) {
  return (uint32_t)(hz * (4294967296.0f / (float)SAMPLE_RATE));
}

static void voiceStart(Voice& v, int midi, const Instrument& I, uint8_t src, bool isBass) {
  v.active = true;
  v.src    = src;
  v.isBass = isBass;
  
  v.wave     = isBass ? W_TRI : I.wave;
  v.oscCount = isBass ? 1 : 2;

  float hz = midiToHz((float)midi);
  for (uint8_t k = 0; k < v.oscCount; k++) {
    float cents = (k == 0) ? -I.det : I.det;
    float f     = hz * powf(2.0f, cents / 1200.0f);
    v.inc[k] = incForHz(f);
    v.dt[k]  = f / (float)SAMPLE_RATE;
    v.ph[k]  = 0;
  }
  v.incSub  = incForHz(hz * 0.5f);
  v.phSub   = 0;
  v.subGain = isBass ? 0.0f : I.sub * 0.5f; 
  v.triZ[0] = v.triZ[1] = 0.0f;

  v.peak         = isBass ? I.sub : 0.16f;    
  v.sustainLevel = v.peak * (I.s > 0.001f ? I.s : 0.001f);
  v.env          = 0.0f;
  v.stage        = ENV_ATTACK;
  v.aStep = v.peak / fmaxf(I.a * SAMPLE_RATE, 1.0f);
  v.dStep = (v.peak - v.sustainLevel) / fmaxf(I.d * SAMPLE_RATE, 1.0f);
  v.rStep = 0.0f;                             


  v.fCut    = I.cut + I.env;
  v.fTarget = fmaxf(120.0f, I.cut);
  float tau = fmaxf((I.a + I.d) * SAMPLE_RATE, 1.0f);
  
  v.fCoefBlk = expf(-3.0f * AUDIO_FRAMES / tau);
  v.fG       = 0.0f;
  v.lp1 = v.lp2 = 0.0f;
}

static void voiceBlockUpdate(Voice& v) {
  v.fCut += (v.fTarget - v.fCut) * (1.0f - v.fCoefBlk);
  float g = 1.0f - expf(-TWO_PI * v.fCut / (float)SAMPLE_RATE);

  v.fG = (g > 0.95f) ? 0.95f : g;
}

static void voiceRelease(Voice& v, float relSec) {
  if (!v.active || v.stage == ENV_RELEASE) return;
  v.stage = ENV_RELEASE;
  v.rStep = v.env / fmaxf(relSec * SAMPLE_RATE, 1.0f);
}


static Voice* voiceAlloc() {
  for (uint8_t i = 0; i < MAX_VOICES; i++) if (!voices[i].active) return &voices[i];

  Voice* best = nullptr;
  for (uint8_t i = 0; i < MAX_VOICES; i++) {
    if (voices[i].stage != ENV_RELEASE) continue;
    if (!best || voices[i].env < best->env) best = &voices[i];
  }
  if (best) return best;

  for (uint8_t i = 0; i < MAX_VOICES; i++)
    if (!best || voices[i].env < best->env) best = &voices[i];
  return best;
}


static inline float voiceRender(Voice& v) {
  switch (v.stage) {
    case ENV_ATTACK:
      v.env += v.aStep;
      if (v.env >= v.peak) { v.env = v.peak; v.stage = ENV_DECAY; }
      break;
    case ENV_DECAY:
      v.env -= v.dStep;
      if (v.env <= v.sustainLevel) { v.env = v.sustainLevel; v.stage = ENV_SUSTAIN; }
      break;
    case ENV_RELEASE:
      v.env -= v.rStep;
      if (v.env <= 0.0f) { v.env = 0.0f; v.active = false; return 0.0f; }
      break;
    default: break;
  }

  float s = 0.0f;
  for (uint8_t k = 0; k < v.oscCount; k++) {
    v.ph[k] += v.inc[k];
    float o = oscShape(v.wave, v.ph[k], v.dt[k]);
    if (v.wave == W_TRI) {                    
      v.triZ[k] += 4.0f * v.dt[k] * o;
      v.triZ[k] *= 0.9995f;                  
      o = v.triZ[k];
    }
    s += o;
  }
  if (v.oscCount == 2) s *= 0.5f;

  if (v.subGain > 0.0f) {
    v.phSub += v.incSub;
    s += oscSine(v.phSub) * v.subGain;
  }

  v.lp1 += (s     - v.lp1) * v.fG;
  v.lp2 += (v.lp1 - v.lp2) * v.fG;

  return v.lp2 * v.env;
}

static void audioTask(void*) {
  const int FRAMES = AUDIO_FRAMES;      
  static int16_t buf[AUDIO_FRAMES * 2]; 

  for (;;) {
   
    AudioEvt e;
    while (xQueueReceive(evtQ, &e, 0) == pdTRUE) {
      const Instrument& I = INSTRUMENTS[e.inst % INSTRUMENT_COUNT];
      if (e.type == EV_CHORD_OFF) {
        for (uint8_t i = 0; i < MAX_VOICES; i++)
          if (voices[i].active && voices[i].src == e.src) voiceRelease(voices[i], I.r);
      } else {
      
        for (uint8_t i = 0; i < MAX_VOICES; i++)
          if (voices[i].active && voices[i].src == e.src) voiceRelease(voices[i], I.r);
        for (uint8_t n = 0; n < e.count; n++) {
          Voice* v = voiceAlloc();
          if (v) voiceStart(*v, e.midi[n], I, e.src, n == e.count - 1);
        }
      }
    }

    uint8_t live = 0;
    for (uint8_t i = 0; i < MAX_VOICES; i++) {
      if (!voices[i].active) continue;
      live++;
      voiceBlockUpdate(voices[i]);    
    }
    gVoicesActive = live;

    float g = gVolume;
    for (int i = 0; i < FRAMES; i++) {
      float mix = 0.0f;
      for (uint8_t vI = 0; vI < MAX_VOICES; vI++)
        if (voices[vI].active) mix += voiceRender(voices[vI]);

      mix *= g;

      float a = fabsf(mix);
      if (a > CLIP_KNEE) {
        float u = (a - CLIP_KNEE) * CLIP_INV_SPAN;
        a = CLIP_KNEE + CLIP_SPAN * (u / (1.0f + u));
        mix = (mix < 0.0f) ? -a : a;
      }

      int16_t o = (int16_t)(mix * 32000.0f);
      buf[i * 2]     = o;                
      buf[i * 2 + 1] = o;               
    }

 
    uint32_t t0 = micros();
    audioWrite(buf, FRAMES);
    uint32_t dt = micros() - t0;
    
    if (dt < (1000000UL * FRAMES / SAMPLE_RATE) / 4) gUnderruns = gUnderruns + 1;
  }
}


static void audioPost(const AudioEvt& e) { if (evtQ) xQueueSend(evtQ, &e, 0); }

static void chordOn(uint8_t src, const SynthState& st, const Chord& c) {
  AudioEvt e = {};
  e.type = EV_CHORD_ON;
  e.src  = src;
  e.inst = st.getInstrumentIndex();
  for (uint8_t i = 0; i < c.count; i++) e.midi[i] = chordMidi(st, c.notes[i]);
  e.midi[c.count] = chordMidi(st, c.bass);  
  e.count = c.count + 1;
  audioPost(e);
}

static void chordOff(uint8_t src, const SynthState& st) {
  AudioEvt e = {};
  e.type = EV_CHORD_OFF;
  e.src  = src;
  e.inst = st.getInstrumentIndex();
  audioPost(e);
}


static const uint8_t SRC_UI = 200;

static void uiBlip(int midi) {
  AudioEvt e = {};
  e.type    = EV_CHORD_ON;
  e.src     = SRC_UI;
  e.inst    = 3;            
  e.count   = 1;
  e.midi[0] = midi;
  audioPost(e);
}
